...
function showUserList() {
    fetch("https://681c33a96ae7c794cf70f666.mockapi.io/users", {
        method: "GET",
    })
        ...
        .then(function (userRes) {
            ...
            <div class="col-md-3 mb-3">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class='d-flex'>
                            <h5 class="card-title me-3">${user.firstName} ${user.lastName}</h5>
...
function selectUsers() {
    let selectUerAll = document.querySelectorAll('input[type="checkbox"]');
    selectUerAll.forEach(function (cb) {
        cb.checked = true;
    });
}
...
async function showDetailUser(userId) {
    let response = await fetch(`https://681c33a96ae7c794cf70f666.mockapi.io/users/${userId}`, {
        method: "GET",
    });
...
async function createUser(e) {
    ...
    //         return;
    //     }
    //     let response = await fetch("https://681c33a96ae7c794cf70f666.mockapi.io/users/", {
    //         method: "POST",
    //         headers: {
    ...
    try {
        ...
        let hasMail = await isExistEmail(user.email);
        if (hasMail) {
            alert(`Email ${user.email} is existed, please using other email !`);
            return;
        }
        ...
        window.alert(`User ${createUserResult.firstName} created success!`);
        showUserList();
        $("#create-user").modal("hide");
    } catch (error) {}
}
...
async function editUser(userId) {
    ...
    document.querySelector("#modify-user #firstname").value = user.firstName;
    document.querySelector("#modify-user #lastname").value = user.lastName;
...
async function updateUser(e) {
    ...
    try {
        ...
        //     return;
        // }
        let response = await fetch(`https://681c33a96ae7c794cf70f666.mockapi.io/users/${user.id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
            },
